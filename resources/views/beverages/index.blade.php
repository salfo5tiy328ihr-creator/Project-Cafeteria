...
<head>
    ...
    <title>Cafeteria - Beverages</title>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .beverage-card:hover .beverage-img-wrapper img {
            transform: scale(1.08);
        }
        .beverage-img-wrapper i.placeholder {
            font-size: 75px;
            background: linear-gradient(135deg, #6366f1, #a855f7);
        ...
        .beverage-card:hover .beverage-img-wrapper i.placeholder {
            transform: scale(1.15) rotate(-5deg);
        }

        .beverage-body { padding: 20px; flex: 1; display: flex; flex-direction: column; }

        .beverage-category-badge {
            display: inline-flex;
            align-items: center;
            gap: 5px;
            font-size: 11px;
        ...
        .beverage-tags { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 14px; }
        ...
        .tag-size { background: rgba(14,165,233,.12); color: #0284c7; }
...
<aside class="sidebar" id="sidebar">
    ...
    <a href="{{ route('food_items.index') }}" class="nav-link">
        <i class="fa-solid fa-burger"></i> Food Items
    </a>
    <a href="{{ route('beverages.index') }}" class="nav-link active">
...
<div class="main">
    ...
    <header class="topbar">
        ...
        <form action="{{ route('beverages.index') }}" method="GET" class="search-box">
            <i class="fa-solid fa-magnifying-glass"></i>
            <input type="text" name="search" placeholder="Search beverages..." value="{{ request('search') }}">
        </form>
    ...
    <div class="content">
        ...
        <div class="hero">
            <div class="row align-items-center g-3">
                <div class="col-lg-7">
                    <h2><i class="fa-solid fa-mug-saucer me-2"></i>Beverages Management</h2>
                    <p>Manage your cafeteria drinks, sizes, and availability.</p>
                </div>
                <div class="col-lg-5 text-lg-end">
                    <a href="{{ route('beverages.create') }}" class="btn-glass">
                        <i class="fa-solid fa-plus"></i> Add New Beverage
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="row g-4 mb-4">
            <div class="col-xl-3 col-md-6">
                <div class="kpi g-indigo d1">
                    ...
                    <div class="label">Total Beverages</div>
                    <div class="value">{{ $beverages->count() }}</div>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="kpi g-emerald d2">
                    ...
                    <div class="value">{{ $beverages->where('is_available', true)->count() }}</div>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="kpi g-rose d3">
                    ...
                    <div class="value">{{ number_format($beverages->sum('calories')) }}</div>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="kpi g-amber d4">
                    ...
                    <div class="value" style="font-size:24px;">
                        ${{ number_format($beverages->sum('price'), 2) }}
                    </div>
                </div>
            </div>
        </div>
            ...
            @forelse($beverages as $beverage)
                <div class="col-xl-3 col-lg-4 col-md-6">
                    <div class="beverage-card">
                        ...
                        <div class="beverage-img-wrapper">
                            @if(!empty($beverage->image))
                                ...
                                @endif
                            @else
                                <i class="fa-solid fa-mug-saucer placeholder"></i>
                            @endif
                        </div>
                        ...
                        <div class="beverage-body">
                            ...
                            @if($beverage->category)
                                <span class="beverage-category-badge">
                                    <i class="fa-solid fa-tag"></i>
                                    {{ $beverage->category->name }}
                                </span>
                            @endif

                            <div class="beverage-title">{{ $beverage->name }}</div>
                            <div class="beverage-desc">
                            ...
                            <div class="beverage-footer">
                                <div class="beverage-price">${{ number_format($beverage->price, 2) }}</div>

                                <div class="action-btns">
                                    <a href="{{ route('beverages.edit', $beverage->id) }}" class="btn-icon" title="Edit">
            ...
            @empty
                <div class="col-12">
                    <div class="empty-state">
                        ...
                        <h5>No Beverages Available</h5>
                        <p>Start by adding your first beverage to the cafeteria menu.</p>
                        <a href="{{ route('beverages.create') }}"
                           class="btn-glass"
                           style="background:linear-gradient(135deg,#6366f1,#a855f7); border:0; display:inline-flex;">
                            <i class="fa-solid fa-plus"></i> Add First Beverage
                        </a>
                    </div>
                </div>
            @endforelse
